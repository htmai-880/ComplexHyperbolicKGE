...
class PoincareConv(MessagePassing):
    def __init__(self, **kwargs):
        ...
        self.w_rel = nn.Linear(3*self.in_channels, 3*self.out_channels, bias=True, dtype=self.data_type)
        ...
        with torch.no_grad():
            ...
            # nn.init.ones_(self.loop_rel[..., self.in_channels_r//2:])

    def forward(self, x, edge_index, edge_type, rel_embed, curvatures):
        if self.device is None:
            self.device = x.device

        trans_rot_r = rel_embed[..., :3 * self.in_channels]
        out_rel = self.w_rel(trans_rot_r)
        ...
        rel1 = expmap0(rel1, loop_curvature)
        rel2 = expmap0(rel2, loop_curvature)
        b_rel1 = expmap0(self.b_rel1, loop_curvature)
        b_rel2 = expmap0(self.b_rel2, loop_curvature)
        rel1 = mobius_add(rel1, b_rel1, loop_curvature)
        rel2 = mobius_add(rel2, b_rel2, loop_curvature)
        rel1 = logmap0(rel1, loop_curvature)
        rel2 = logmap0(rel2, loop_curvature)
        ...
        out = self.propagate(edge_index,   x=x, edge_type=edge_type,   rel_embed=out_rel, curvatures=curvatures)
        # Nonlinearity
...
class PoincareGCN(KGModel):
    def __init__(self, args, dataset):
        ...
        if self.multi_c:
            self.c_layer = nn.Embedding(self.sizes[1], args.layers)
            self.c = nn.Embedding(self.sizes[1], 1)
        else:
            self.c_layer = nn.Embedding(1, args.layers)
    ...
    def forward_base(self):
        ...
        if self.multi_c:
            curvatures = F.softplus(self.c_layer.weight)
        ...
        for i, conv in enumerate(self.layers):
            x, r = conv.forward(x, edge_index, edge_type, r, curvatures[..., i:i+1] if self.multi_c else None)
        del edge_index
        del edge_type
        return (x, r, F.softplus(self.c.weight))
