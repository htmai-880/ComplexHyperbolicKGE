...
def expmap0_lorentz(u, c):
    """Exponential map taken at the origin of the hyperboloid with curvature c.
    ...
    """
    sqrt_c = c ** 0.5
    alpha = sqrt_c * u.norm(dim=-1, p=2, keepdim=True).clamp_min(MIN_NORM)
    return (torch.sinh(alpha) / alpha) * u

def logmap0_lorentz(y, c):
    """Logarithmic map taken at the origin of the Poincare ball with curvature c.
...
def lorentz_boost(y, v, c):
    ...
    Args:
        y: torch.Tensor of size B x d with tangent points
        v: torch.Tensor of size B x d with velocity vectors
        c: torch.Tensor of size 1 or B x 1 with absolute hyperbolic curvatures
...
def hyp_distance_multi_c_lorentz(x, v, c):
    """Hyperbolic distance on hyerboloid with varying curvatures c.
    ...
    """
    x0 = torch.sum(x**2, dim=-1, keepdim=True) + 1/c # (B, 1, 1) during train and eval
    x0 = torch.sqrt(x0) # (B, 1, 1)
    v0 = torch.sum(v**2, dim=-1, keepdim=True) + 1/c # (B, eta, 1) during train, (1/B, N, 1) during eval
    v0 = torch.sqrt(v0) # (B, eta, 1) or (B, N, 1)
